...
namespace UserControlMissions
        {
            public partial class UserControlMission : UserControl
            {
                ...
                public UserControlMission(DataSet ds, int i, SQLiteConnection connec)
                {
                    ...
                    if (ds.Tables["Mission"].Rows[i][7].ToString() == "1")
                    {
                ...
                private void pctRes_Click(object sender, EventArgs e)
                {
                    if (ds.Tables["Mission"].Select($"id = {(indexMission + 1)} AND terminee = 0").Length > 0)
                    {
                        ...
                        //Les engins
                        var enginAdapter = new SQLiteDataAdapter("SELECT * FROM Engin", Connexion);
                        var enginBuilder = new SQLiteCommandBuilder(enginAdapter);
                        enginAdapter.Update(ds, "Engin");

                        // Les pompiers
                        var pompierAdapter = new SQLiteDataAdapter("SELECT * FROM Pompier", Connexion);
                        var pompierBuilder = new SQLiteCommandBuilder(pompierAdapter);
                        pompierAdapter.Update(ds, "Pompier");

                        // Mobiliser
                        var mobiliserAdapter = new SQLiteDataAdapter("SELECT * FROM Mobiliser", Connexion);
                        var mobiliserBuilder = new SQLiteCommandBuilder(mobiliserAdapter);
                        mobiliserAdapter.Update(ds, "Mobiliser");

                        //PartirAvec
                        var partirAvecAdapter = new SQLiteDataAdapter("SELECT * FROM PartirAvec", Connexion);
                        var partirAvecBuilder = new SQLiteCommandBuilder(partirAvecAdapter);
                        partirAvecAdapter.Update(ds, "PartirAvec");
                    }
                    else
                    {
